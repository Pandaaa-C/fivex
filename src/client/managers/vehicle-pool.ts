...
export class VehiclePool extends ClientPool<Vehicle> {
    ...
    async new(model: string | number, position: IVector3): Promise<Vehicle | null> {
        const hash = typeof model === "string" ? GetHashKey(model) : model;

        RequestModel(hash);
        while (!HasModelLoaded(hash)) await new Promise((resolve) => setTimeout(resolve, 0));
		
        const handle = CreateVehicle(hash, position.x, position.y, position.z, 0, false, false);
